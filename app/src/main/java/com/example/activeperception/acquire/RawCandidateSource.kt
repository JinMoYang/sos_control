...
class RawFrame(
    ...
    val whiteBalance: FloatArray = floatArrayOf(1f, 1f, 1f),
    val cameraToSrgb: DoubleArray = Formation.IDENTITY_3X3
)
...
class RawCandidateSource(
    ...
    override fun render(cells: IntArray): List<Bitmap> {
        ...
        return cells.map { c ->
            ...
            val argb = Formation.packCandidateArgb(rgb[0], rgb[1], rgb[2],
                grid.gainRatio(gi), maxDn, calibration.whiteBalance, calibration.cameraToSrgb)
            val oriented = Formation.orientArgb(argb, ow, oh, calibration.sensorOrientation)
